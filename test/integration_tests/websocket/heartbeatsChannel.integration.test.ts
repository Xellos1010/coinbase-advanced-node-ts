// test/integration_tests/websocket/heartbeatsChannel.integration.test.ts
...
describe('Coinbase API Integration Test - WebSocket Heartbeats Channel', () => {
  ...
  it('should connect to WebSocket server and subscribe to heartbeats channel', async () => {
    await testChannelSubscription(wsClient, 'heartbeats', 'coinbaseIntegrationTest_websocket_heartbeats', onMessageSpy);
  });
});
